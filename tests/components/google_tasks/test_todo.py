...
@pytest.mark.parametrize(
    ...
    [
        ("Remind me to call Harsha on 2024/10/23", datetime(2024, 10, 23)),
        ("Remind me to schedule a meeting on 2024/12/24", datetime(2024, 12, 24)),
        ("Remind me to wish happy new year on 2026/01/01", datetime(2026, 1, 1)),
        ("I have a badminton match on 2023/12/01", datetime(2023, 12, 1)),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ("Remind me to call Harsha", "Call Harsha on the 23rd", datetime(2024, 10, 23)),
        ("Meeting with team", "Schedule meeting on 24th Dec", datetime(2024, 12, 24)),
        ("New Year Reminder", "Wish happy new year on Jan 1st", datetime(2026, 1, 1)),
    ],
)
...
async def test_integration_create_todo_item() -> None:
    ...
    # Mocking the coordinator with sample data
    mock_coordinator = Mock()

    # Sample data simulating existing tasks
    mock_coordinator.data = [
        create_todo_item(
            "Remind me to call Harsha on 2024/10/23",
            datetime(2024, 10, 23),
            position="1",
        ),
        create_todo_item(
    ...
    # Creating the GoogleTaskTodoListEntity instance
    ...
    # Retrieve current todo items
    todo_items = task_entity.todo_items

    # Assertions to verify the items are as expected
