...
def _async_register_events_and_services(hass: HomeAssistant) -> None:
    ...
    def _get_running_homekit_instances() -> list:
        """Retrieve all running HomeKit instances."""
        return [
            homekit
    ...
    async def async_reset_accessories(service: ServiceCall) -> None:
        """Reset HomeKit accessories for specified entity IDs."""
        entity_ids = cast(list[str], service.data.get("entity_id"))
        running_homekits = _get_running_homekit_instances()

        if not running_homekits:
            _LOGGER.warning("No running HomeKit instances found.")
            return

        for homekit in running_homekits:
            await homekit.async_reset_accessories(entity_ids)

    async def async_unpair_devices(service: ServiceCall) -> None:
        """Unpair devices from HomeKit."""
    ...
    async def async_reload_homekit(service: ServiceCall) -> None:
        """Reload HomeKit integration."""
        config = await async_integration_yaml_config(hass, DOMAIN)
        if not config or DOMAIN not in config:
            _LOGGER.warning("No HomeKit configuration found in YAML.")
            return
