...
def _is_priority_task(title: str, description: str | None) -> bool:
    """Check if the task contains priority keywords."""
...
def _convert_todo_item(item: TodoItem) -> dict[str, str | None]:
    ...
    result["priority"] = (
        "high" if _is_priority_task(item.summary, item.description) else "normal"
    )
    return result
...
def _order_tasks(tasks: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    parents.sort(
        key=lambda task: (
            not _is_priority_task(task["title"], task.get("notes")),
            task["position"],
        )
    )
    return parents
