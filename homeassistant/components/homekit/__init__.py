...
def _async_register_events_and_services(hass: HomeAssistant) -> None:
    ...
    _register_reset_accessory_service(hass)
    _register_unpair_service(hass)
    _register_reload_service(hass)


def _register_reset_accessory_service(hass: HomeAssistant) -> None:
    """Register the reset accessory service for HomeKit."""
    async def _handle_reset_accessory(service: ServiceCall) -> None:
        """Handle reset accessory HomeKit service call."""
        for homekit in _get_running_homekit_instances(hass):
    ...
    hass.services.async_register(
        DOMAIN,
        SERVICE_HOMEKIT_RESET_ACCESSORY,
        _handle_reset_accessory,
        schema=RESET_ACCESSORY_SERVICE_SCHEMA,
    )


def _register_unpair_service(hass: HomeAssistant) -> None:
    """Register the unpair service for HomeKit."""
    async def _handle_unpair(service: ServiceCall) -> None:
        """Handle unpair HomeKit service call."""
        referenced_devices = async_extract_referenced_entity_ids(hass, service).referenced_devices
        dev_reg = dr.async_get(hass)

        for device_id in referenced_devices:
            await _unpair_device(device_id, dev_reg, hass)
...
async def _unpair_device(device_id: str, dev_reg: dr.DeviceRegistry, hass: HomeAssistant) -> None:
    """Unpair a device from HomeKit."""
    dev_reg_ent = dev_reg.async_get(device_id)
    if not dev_reg_ent:
        raise HomeAssistantError(f"No device found for device id: {device_id}")

    mac_addresses = _get_device_mac_addresses(dev_reg_ent)

    matching_instances = _find_matching_homekit_instances(macs=mac_addresses, hass=hass)
    if not matching_instances:
        raise HomeAssistantError(f"No HomeKit accessory found for device id: {device_id}")

    for homekit in matching_instances:
        await homekit.async_unpair()


def _get_device_mac_addresses(dev_reg_ent: dr.DeviceEntry) -> list[str]:
    """Retrieve MAC addresses from a device registry entry."""
    return [
        cval
...
def _find_matching_homekit_instances(macs: list[str], hass: HomeAssistant) -> list:
    """Find HomeKit instances that match the given MAC addresses."""
    return [
        homekit
...
def _register_reload_service(hass: HomeAssistant) -> None:
    """Register the reload service for HomeKit."""
    async def _handle_reload(service: ServiceCall) -> None:
        ...
        await _reload_homekit_instances(hass, config)
...
async def _reload_homekit_instances(hass: HomeAssistant, config: dict) -> None:
    """Reload HomeKit instances from the given configuration."""
    current_entries = hass.config_entries.async_entries(DOMAIN)
...
def _get_running_homekit_instances(hass: HomeAssistant) -> list:
    """Retrieve HomeKit instances that are running."""
    return [
        homekit
        for homekit in _async_all_homekit_instances(hass)
        if homekit.status == STATUS_RUNNING
